    ...
    public class WItem
    {
        ...
        public void checkDB()
        {
            ...
            try
            {
                ...
                {
                    wPlayer.hasItems.RemoveAll((WItem x) => x.id == id);
                }
            }
            catch
            {
        ...
        public string ItemMess()
        {
            ...
            if (Level < WeaponPlus.config.MaximunofLevel)
            {
                ...
                handler.AppendFormatted(plusPrice(PlusType.damage, out var price) ? WeaponPlus.cointostring(price, out List<Item> items) : WeaponPlus.LangTipsGet("当前已满级"));
                stringBuilder6.AppendLine(ref handler);
                stringBuilder2 = stringBuilder;
                StringBuilder stringBuilder7 = stringBuilder2;
                handler = new StringBuilder.AppendInterpolatedStringHandler(0, 2, stringBuilder2);
                handler.AppendFormatted(WeaponPlus.LangTipsGet("大小升至下一级需："));
                handler.AppendFormatted(plusPrice(PlusType.scale, out price) ? WeaponPlus.cointostring(price, out items) : WeaponPlus.LangTipsGet("当前已满级"));
                stringBuilder7.AppendLine(ref handler);
                stringBuilder2 = stringBuilder;
                StringBuilder stringBuilder8 = stringBuilder2;
                handler = new StringBuilder.AppendInterpolatedStringHandler(0, 2, stringBuilder2);
                handler.AppendFormatted(WeaponPlus.LangTipsGet("击退升至下一级需："));
                handler.AppendFormatted(plusPrice(PlusType.knockBack, out price) ? WeaponPlus.cointostring(price, out items) : WeaponPlus.LangTipsGet("当前已满级"));
                stringBuilder8.AppendLine(ref handler);
                stringBuilder2 = stringBuilder;
                StringBuilder stringBuilder9 = stringBuilder2;
                handler = new StringBuilder.AppendInterpolatedStringHandler(0, 2, stringBuilder2);
                handler.AppendFormatted(WeaponPlus.LangTipsGet("攻速升至下一级需："));
                handler.AppendFormatted(plusPrice(PlusType.useSpeed, out price) ? WeaponPlus.cointostring(price, out items) : WeaponPlus.LangTipsGet("当前已满级"));
                stringBuilder9.AppendLine(ref handler);
                stringBuilder2 = stringBuilder;
                StringBuilder stringBuilder10 = stringBuilder2;
                handler = new StringBuilder.AppendInterpolatedStringHandler(0, 2, stringBuilder2);
                handler.AppendFormatted(WeaponPlus.LangTipsGet("射弹飞速升至下一级需："));
                handler.AppendFormatted(plusPrice(PlusType.shootSpeed, out price) ? WeaponPlus.cointostring(price, out items) : WeaponPlus.LangTipsGet("当前已满级"));
        ...
        public bool plusPrice(PlusType plus, out long price, int addlevel = 1)
        {
            Item itemById = TShock.Utils.GetItemById(id);
            price = 0L;
            if (Level + addlevel > WeaponPlus.config.MaximunofLevel)
            {
                return false;
            }
                    ...
                    case PlusType.damage:
                        {
                            ...
                            {
                                num3 = 1.0 / orig_damage * (num * 20.0 * num2);
                                num5 = damage_level + i;
                            }
                    ...
                    case PlusType.useSpeed:
                        {
                            ...
                            num3 = (orig_useAnimation * 1.0 / (orig_useAnimation - (useSpeed_level + i * 1.0)) - orig_useAnimation * 1.0 / (orig_useAnimation - (useSpeed_level + i - 1.0))) * (num * 20.0 * num2);
                            num3 *= 1.05;

...
public class Plugin : TerrariaPlugin
{
    ...
    private void PluginManager(CommandArgs args)
    {
        ...
        else if (args.Parameters.Count >= 1 && (args.Parameters[0].ToLower() == "-p" || args.Parameters[0].ToLower() == "p"))
        {
            switch (args.Parameters.Count)
            {
                case 1:
                    args.Player.SendInfoMessage("有以下平台可以切换:");
                    var count = 1;
                    foreach (var (platform, _) in Config._names)
                    {
                        args.Player.SendInfoMessage($"{count}.{platform}");
                        count++;
                    }
                    args.Player.SendInfoMessage("使用/apm -p [序号] 命令切换插件安装平台!");
                    break;
                default:
                    if (!int.TryParse(args.Parameters[1], out var index))
                    {
                        args.Player.SendErrorMessage("输入了错误的序号!");
                        return;
                    }
                    var plats = Config._names.Values.ToList();
                    if (index < 1 && index > plats.Count)
                    {
                        args.Player.SendErrorMessage("不存在的序号!");
                        return;
                    }
                    Config.PluginConfig.RunPlatform = plats[index - 1];
                    Config.PluginConfig.Write();
                    args.Player.SendSuccessMessage($"已将插件安装平台切换至 {Config.GetPlatformName(plats[index - 1])}");
                    break;
            }
        }
        else
        {
            ...
            args.Player.SendInfoMessage(GetString("apm p 切换安装平台"));

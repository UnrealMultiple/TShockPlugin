...
{
    private static string GiteeReleaseUrl => $"https://gitee.com/kksjsj/TShockPlugin/releases/download/V1.0.0.0/Plugins_{Config.GetPlatformName(Config.PluginConfig.RunPlatform)}.zip";
    ...
    public static void LoadPlugins(IEnumerable<PluginVersionInfo> Plugins)
    {
    ...
    public static List<PluginUpdateInfo> GetUpdates(List<PluginVersionInfo> latestPluginList)
    {
        List<PluginUpdateInfo> pluginUpdateList = new();
        var plugins = GetPlugins();
        foreach (var latestPluginInfo in latestPluginList)

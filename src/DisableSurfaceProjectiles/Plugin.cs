...
public class Plugin : TerrariaPlugin
{
    ...
    public void Command(CommandArgs args)
    {
        ...
        var previousState = _isEnabled; // 记录之前的启用状态
        ...
        if (_isEnabled)
        {
            ...
            if (projectileIdsWithNames != null)
            {
                ...
                string FormatItem(int index, KeyValuePair<int, string> kv)
                {
                    ...
                    var separator = (index + 1) % 5 == 0 && index < projectileIdsWithNames.Count - 1 ? "\n" : "";
                    return $" {itemNumber}[c/EEF992:{kv.Value}]([c/73DCE6:{kv.Key}]){separator}";
                }

                // 构造一个格式化的弹幕列表，供玩家查看，仅在开启时显示：不想给它加命令，所以干脆写在里面了
                var count = projectileIdsWithNames.Count;
                var formattedIdsForPlayers = string.Join("", Enumerable.Range(0, count).Select(index => FormatItem(index, projectileIdsWithNames.ElementAt(index))));

                // 移除列表末尾的多余换行符（如果存在）
                if (formattedIdsForPlayers.EndsWith("\n") && formattedIdsForPlayers.Length > 1)
                {
                    formattedIdsForPlayers = formattedIdsForPlayers[..^1];
                }
                ...
                var fullConsoleMessage = $"{consoleMessagePrefix}{consoleProjectilesList}";
    ...
    private void OnProjectileNew(object? sender, GetDataHandlers.NewProjectileEventArgs e)
    {
        ...
        this.CheckAndHandleNormalHeightRestriction(e);
        this.CheckAndHandleInvertedHeightRestriction(e);
    }
}
...
{
    public static PlayerRecordBase<T>.Context PlayerContext<T>(string tableName) where T : PlayerRecordBase<T>
    {
    ...
    public static DisposableQuery<T> Get<T>(string name, string tableName) where T : PlayerRecordBase<T>
    {
        var context = PlayerRecordBase<T>.GetContext(tableName);
        return new DisposableQuery<T>(context.Get(name), context);
    }

    public static DisposableQuery<T> Get<T>(this TSPlayer player, string tableName) where T : PlayerRecordBase<T>
    {

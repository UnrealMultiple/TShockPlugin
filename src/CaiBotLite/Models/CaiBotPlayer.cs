...
public partial class CaiBotPlayer : TSPlayer
{
    ...
    public override void SendMessage(string msg, byte red, byte green, byte blue)
    {
        ...
        var result = regex.Replace(result1, m =>
        {
            ...
            if (stack == "1")
            {
                return prefix == "0" ? $"[{Lang.GetItemName(int.Parse(netId))}]" : $"[{Lang.prefix[int.Parse(prefix)]} {Lang.GetItemName(int.Parse(netId))}]"; //return $"[{Terraria.Lang.prefix[int.Parse(netID)]}]";
            }

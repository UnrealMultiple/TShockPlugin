...
public class CrystalLeafShot : CProjectile
{
    ...
    public override void MyEffect()
    {
        try
        {
            ...
            {
                var num = Collect.MyNewProjectile(Projectile.GetNoneSource(), Main.player[this.proj.owner].Center + new Vector2(0f, -60f), this.proj.velocity.RotatedBy(-0.05), 227, 75, 5f, this.proj.owner);
                Collect.Cprojs[num] = new CrystalLeafShot(Main.projectile[num], new float[0], 1);
                Update(num);
                num = Collect.MyNewProjectile(Projectile.GetNoneSource(), Main.player[this.proj.owner].Center + new Vector2(0f, -60f), this.proj.velocity.RotatedBy(0.05), 227, 75, 5f, this.proj.owner);
                Collect.Cprojs[num] = new CrystalLeafShot(Main.projectile[num], new float[0], 1);
                Update(num);
            }
        }

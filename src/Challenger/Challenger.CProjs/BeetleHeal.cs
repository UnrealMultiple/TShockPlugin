        ...
        if (this.proj.active && this.proj.timeLeft <= this.ai[1] - 60f)
        {
            ...
            try
            {
                ...
                foreach (var val2 in players)
                {
                    ...
                    if (val3.LengthSquared() <= val2.TPlayer.width * val2.TPlayer.height)
                    {
                        Challenger.HealPlayer(val2.TPlayer, num, visible: false);
                        val2.SetBuff(95, 300);
                        if (Challenger.Config.EnableConsumptionMode)
                        {
                            Challenger.SendPlayerText(GetString($"甲虫治疗 + {num} 治疗者:{Main.player[this.proj.owner].name}"), new (210, 0, 255), val2.TPlayer.Center + new Vector2(Main.rand.Next(-60, 61), Main.rand.Next(61)));
                            val2.SendMessage(GetString($"你被 {Main.player[this.proj.owner].name} 治疗了 {num} 点生命值"), new (210, 0, 255));
                        }
                        else
                        {
                            Challenger.SendPlayerText($"{num}", new (0, 255, 0), val2.TPlayer.Center + new Vector2(Main.rand.Next(-60, 61), Main.rand.Next(61)));
                        }
                        this.CKill();
                        return;
                    }

  ...
  "技能列表": [
    {
      ...
      "执行脚本": "环绕锁敌.js", //执行脚本名称
      "重置变量": false //每次触发是否重置脚本全局变量（true=归零，false=保留）
    }
  ]
}
```
...
### 脚本执行与重载
- 脚本文件只会在 **首次触发** 时读取、编译并运行一次顶层代码（定义 `main` 与全局环境）。之后每次触发仅调用 `main` 函数，不会重复编译或重复执行顶层代码，也不会反复从磁盘读取文件。
- **是否重置变量**（`重置变量`）：默认为 `false`，顶层全局变量会在多次触发间保留；设为 `true` 时，每次触发前会恢复为干净全局、重新执行顶层代码，顶层变量每次归零（不重新解析）。
- 因为引擎被长期保留，脚本里通过 `Schedule` 排入延时的回调（会引用脚本作用域中的变量）在后续的游戏帧依然有效。
- 修改脚本文件后执行 `/reload`：插件只会对 **发生变化** 的脚本文件重新读取并重新编译；未改动的脚本保持原有引擎不变。若脚本文件缺失或为空，该技能会被跳过（不会反复报错）。- 每个技能脚本拥有独立的引擎，彼此隔离；但单个角色技能的多次触发会复用同一个引擎。

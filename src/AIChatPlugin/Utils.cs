    ...
    internal class Utils
    {
        ...
        public static void ChatWithAI(TSPlayer player, string question)
        {
            ...
            Task.Run(async () =>
            {
                ...
                {
                    string Requesterror = $"[AIChatPlugin] 处理请求时发生错误！详细信息：{ex.Message}";
                    TShock.Log.ConsoleError(Requesterror);
                    if (player.IsLoggedIn)
                    {
                        player.SendErrorMessage(Requesterror);
                    }
                }
                finally
                {
        ...
        public static async Task ProcessAIChat(TSPlayer player, string question)
        {
            try
            {
                ...
                string formattedContext = context.Count > 0 ? "上下文信息:\n" + string.Join("\n", context) + "\n\n" : "";
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    string taskId = result?.Id ?? "Not provided";
                    ...
                    if (result != null && result.Choices != null && result.Choices.Length > 0)
                    {
                        ...
                        string formattedQuestion = FormatMessage(question);
                        string formattedResponse = FormatMessage(responseMessage);
                    ...
                    {
                        string Novalidresponse = "[AIChatPlugin] 很抱歉，这次未获得有效的AI响应";
                        TShock.Log.ConsoleError(Novalidresponse);
                        if (player.IsLoggedIn)
                        {
                            player.SendErrorMessage(Novalidresponse);
                        }
                    }
                }
                else
                {
                    string Noresponse = $"[AIChatPlugin] AI未能及时响应，状态码：{response.StatusCode}";
                    TShock.Log.ConsoleError(Noresponse);
                    if (player.IsLoggedIn)
                    {
                        player.SendErrorMessage(Noresponse);
                    }
                }
            }
            catch (TaskCanceledException)
            {
                string Timeout = "[AIChatPlugin] 请求超时！";
                TShock.Log.ConsoleError(Timeout);
                if (player.IsLoggedIn)
                {
                    player.SendErrorMessage(Timeout);
                }
            }
            catch (Exception ex)
            {
                string AIError = $"[AIChatPlugin] 出现错误！详细信息：{ex.Message}";
                TShock.Log.ConsoleError(AIError);
                if (player.IsLoggedIn)
                {
                    player.SendErrorMessage(AIError);
                }
            }
        }

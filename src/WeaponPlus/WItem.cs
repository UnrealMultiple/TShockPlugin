...
public class WItem
{
    ...
    public void checkDB()
    {
        ...
        try
        {
            ...
            {
                wPlayer.hasItems.RemoveAll((WItem x) => x.id == this.id);
            }
        }
        catch
        {
    ...
    public string ItemMess()
    {
        ...
        {
            num = this.damage_level;
        }
        var value = num * 1f / this.orig_damage;
        stringBuilder2 = stringBuilder;
        ...
        value = (this.orig_useAnimation * 1f / (this.orig_useAnimation - this.useSpeed_level)) - 1f;
        stringBuilder2 = stringBuilder;
        ...
        if (this.Level < WeaponPlus.config.MaximunofLevel)
        {
            ...
            handler.AppendFormatted(this.plusPrice(PlusType.damage, out var price) ? WeaponPlus.cointostring(price, out var items) : WeaponPlus.LangTipsGet("当前已满级"));
            stringBuilder6.AppendLine(ref handler);
            stringBuilder2 = stringBuilder;
            var stringBuilder7 = stringBuilder2;
            handler = new StringBuilder.AppendInterpolatedStringHandler(0, 2, stringBuilder2);
            handler.AppendFormatted(WeaponPlus.LangTipsGet("大小升至下一级需："));
            handler.AppendFormatted(this.plusPrice(PlusType.scale, out price) ? WeaponPlus.cointostring(price, out items) : WeaponPlus.LangTipsGet("当前已满级"));
            stringBuilder7.AppendLine(ref handler);
            stringBuilder2 = stringBuilder;
            var stringBuilder8 = stringBuilder2;
            handler = new StringBuilder.AppendInterpolatedStringHandler(0, 2, stringBuilder2);
            handler.AppendFormatted(WeaponPlus.LangTipsGet("击退升至下一级需："));
            handler.AppendFormatted(this.plusPrice(PlusType.knockBack, out price) ? WeaponPlus.cointostring(price, out items) : WeaponPlus.LangTipsGet("当前已满级"));
            stringBuilder8.AppendLine(ref handler);
            stringBuilder2 = stringBuilder;
            var stringBuilder9 = stringBuilder2;
            handler = new StringBuilder.AppendInterpolatedStringHandler(0, 2, stringBuilder2);
            handler.AppendFormatted(WeaponPlus.LangTipsGet("攻速升至下一级需："));
            handler.AppendFormatted(this.plusPrice(PlusType.useSpeed, out price) ? WeaponPlus.cointostring(price, out items) : WeaponPlus.LangTipsGet("当前已满级"));
            stringBuilder9.AppendLine(ref handler);
            stringBuilder2 = stringBuilder;
            var stringBuilder10 = stringBuilder2;
            handler = new StringBuilder.AppendInterpolatedStringHandler(0, 2, stringBuilder2);
            handler.AppendFormatted(WeaponPlus.LangTipsGet("射弹飞速升至下一级需："));
            handler.AppendFormatted(this.plusPrice(PlusType.shootSpeed, out price) ? WeaponPlus.cointostring(price, out items) : WeaponPlus.LangTipsGet("当前已满级"));
    ...
    #region 强化价格
    public bool plusPrice(PlusType plus, out long price, int addlevel = 1)
    {
        var itemById = TShock.Utils.GetItemById(this.id);
        price = 0L;
        if (this.Level + addlevel > WeaponPlus.config.MaximunofLevel)
        {
            return false;
        }
        for (var i = 1; i <= addlevel; i++)
        {
            ...
            switch (plus)
            {
                case PlusType.damage:
                {
                    ...
                    {
                        num3 = 1.0 / this.orig_damage * (num * 20.0 * num2);
                ...
                case PlusType.useSpeed:
                {
                    ...
                    num3 = ((this.orig_useAnimation * 1.0 / (this.orig_useAnimation - (this.useSpeed_level + (i * 1.0)))) - (this.orig_useAnimation * 1.0 / (this.orig_useAnimation - (this.useSpeed_level + i - 1.0)))) * (num * 20.0 * num2);
                    num3 *= 1.05;

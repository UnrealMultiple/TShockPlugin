        ...
        switch (cmd)
        {
                    ...
                    if (authorHouses < maxHouses || args.Player.Group.HasPermission("house.bypasscount"))
                    {
                        if (!args.Player.TempPoints.Any(p => p == Point.Zero))
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    args.Player.SendErrorMessage("因为您的房子总面积超过了最大限制 " + maxSize.ToString() + " 格块。");
                                }

                                if (width < LConfig.MinWidth)
                                {
                                    args.Player.SendErrorMessage("因为您的房屋宽度小于最小限制 " + LConfig.MinWidth.ToString() + " 格块。");
                                }

                                if (height < LConfig.MinHeight)
                                {
                                    args.Player.SendErrorMessage("因为您的房屋高度小于最小限制 " + LConfig.MinHeight.ToString() + " 格块。");
                                }
                            }
                        }
                        else
            ...
            case "allow"://添加所有者
            {
                if (args.Parameters.Count > 2)
                {
                    ...
                    {
                        args.Player.SendErrorMessage("你没有权力分享这个房子!");//只有房子的作者可以
                    }
                }
                else
                {
            ...
            case "disallow":
            {
                if (args.Parameters.Count > 2)
                {
                    ...
                    {
                        args.Player.SendErrorMessage("你没有权力管理这个房子!");//只有房子的作者可以
                    }
                }
                else
                {
            ...
            case "delete":
            {
                if (args.Parameters.Count > 1)
                {
                    ...
                    {
                        args.Player.SendErrorMessage("你没有权力删除这个房子!");//只有房子的作者可以
                    }
                }
                else
                {
            ...
            case "redefine"://重新定义
            {
                if (args.Parameters.Count > 1)
                {
                    if (!args.Player.TempPoints.Any(p => p == Point.Zero))
                    {
                        ...
                        {
                            args.Player.SendErrorMessage("你没有权力修改这个房子!");//只有房子的作者可以
                        }
                    }
                    else
                    {

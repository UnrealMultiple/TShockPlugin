...
public class WeaponPlus : TerrariaPlugin
{
    ...
    private void PlusItem(CommandArgs args)
    {
        ...
        else if (args.Parameters.Count == 1)
        {
            ...
            else if (args.Parameters[0].Equals("clear", StringComparison.OrdinalIgnoreCase))
            {
                ...
                args.Player.SendMessage(GetString("完全重置成功！" + EconomicsAPI.Economics.Setting.CurrencyName + "回收：") + num, new Color(0, 255, 0));
            }
            else
            {
    ...
    public bool Deduction(WItem WItem, int whoAMI, PlusType plusType, int gap = 1)
    {
        ...
        TShock.Players[whoAMI].SendInfoMessage(GetString(EconomicsAPI.Economics.Setting.CurrencyName + "不足！"));
        return false;
    }
    #endregion

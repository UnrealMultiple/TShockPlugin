    ...
    private void PCO(CommandArgs args)
    {
        if (args.Parameters.Count == 0)
        {
            ...
            if (config.OpenAutoControlProgressLock)
            {
                ...
                var mess = args.Player.IsLoggedIn ? "[i:3868]已解锁Boss：\n" : "@已解锁Boss：\n";
                var count = 0;
                //把排好序的数据输出
                ...
                else
                {
                    ...
                    else
                    {
                        mess_reset = config.OpenAutoReset && !countdownReset.enable
                            ? $"世界将在{HoursToM((config.StartServerDate.AddHours(config.HowLongTimeOfAotuResetServer) - DateTime.Now).TotalHours, (args.Player.IsLoggedIn ? "EA00FF" : ""))}后开始自动重置\n"
                            : $"世界将在{HoursToM(countdownReset.time * 1.0 / 3600, (args.Player.IsLoggedIn ? "EA00FF" : ""))}后开始手动重置\n";
                    }
                    ...
                    else
                    {
                        mess_reload = config.AutoRestartServer && !countdownRestart.enable
                            ? $"服务器将在{HoursToM((config.LasetServerRestartDate.AddHours(config.HowLongTimeOfRestartServer) - DateTime.Now).TotalHours, (args.Player.IsLoggedIn ? "FF9000" : ""))}后开始自动重启\n"
                            : $"服务器将在{HoursToM(countdownRestart.time * 1.0 / 3600, (args.Player.IsLoggedIn ? "FF9000" : ""))}后开始手动重启\n";
                    }
                    ...
                    else
                    {
                        mess_com = config.OpenAutoCommand && !countdownCom.enable
                            ? $"服务器将在{HoursToM((config.LasetAutoCommandDate.AddHours(config.HowLongTimeOfAutoCommand) - DateTime.Now).TotalHours, (args.Player.IsLoggedIn ? "00A8FF" : ""))}后开始自动执行指令\n"
                            : $"服务器将在{HoursToM(countdownCom.time * 1.0 / 3600, (args.Player.IsLoggedIn ? "00A8FF" : ""))}后开始手动执行指令\n";
                    }

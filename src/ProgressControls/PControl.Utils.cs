    ...
    public partial class PControl : TerrariaPlugin
    {
        ...
        private static async void ResetGame()
        {
            ...
            try
            {
                ...
                {
                    foreach (string v in config.DeleteFileForBeforeResetting)
                    {
            ...
            */
            ...
            if (config.ExpectedUsageWorldFileNameForAotuReset.Count > 0 && ExistWorldNamePlus(config.ExpectedUsageWorldFileNameForAotuReset.First(), out string worldname))
            {
        ...
        {
            double jiange = (DateTime.Now - config.StartServerDate).TotalHours;
        ...
        private void Function(TSPlayer player, string bossname, double addtime)
        {
            ...
            if (addtime > 0)
                ...
                st = "提前" + (player.IsLoggedIn ? HoursToM(-1 * addtime, "28FFB8") : HoursToM(-1 * addtime));
            else
                st = "正常";
        ...
        private static string HoursToM(double hours, string color = "")
        {
            ...
            if (hours >= 0)
            {
                ...
                h = (int)hours;
                m = (int)(hours % 1 * 60);//分
                double tempS = (hours % 1 * 60 % 1 * 60);//秒
                if (tempS % 1 >= 0.5)//四舍五入
                    s = (int)tempS + 1;
                else
                    s = (int)tempS;
        ...
        {
            failureReason = "";
            if (double.TryParse(preTime, out double hour))
            {
                return hour;
            }
            bool has负号 = false;
            List<string> list = preTime.Split(':', '.').ToList();
            list.ForEach(x =>
            {
            ...
            else
            {
                int h = 0, m = 0, s = 0;
                double sum;
                if (int.TryParse(list[0], out h) || string.IsNullOrWhiteSpace(list[0]) || list[0] == "-" || list[0] == "+")
                {
                    if (int.TryParse(list[1], out m) || string.IsNullOrWhiteSpace(list[1]))
                    {
                        if (int.TryParse(list[2], out s) || string.IsNullOrWhiteSpace(list[2]))
                        {
                            ...
                            if (has负号)
                                sum = (-1 * h + m / 60.0 + s / 3600.0) * -1;
                            else
                                sum = h + m / 60.0 + s / 3600.0;
                            return sum;
                        }
                        else
                        {

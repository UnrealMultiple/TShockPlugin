...
namespace ProgressControl
{
    ...
    public partial class PControl : TerrariaPlugin
    {
        ...
        /// NPC死亡次数触发器
        /// </summary>
        private static Dictionary<int, bool> NpcAndKillCountTrigger = new Dictionary<int, bool>();

        #region 线程
        /// <summary>
        /// 自动计划线程，包括自动重置，自动重启，自动执行指令
        /// </summary>
        private Thread thread_auto = new Thread(thread_autoFun);
        private static void thread_autoFun(object? obj)
        {
            while (!Netplay.Disconnect)
            {
                ...
                {
                    TimeSpan span = config.StartServerDate.AddHours(config.HowLongTimeOfAotuResetServer) - DateTime.Now;
                    double time_temp = span.TotalSeconds % 1;
                    int time;
                    if (time_temp >= 0.5)
                        time = (int)span.TotalSeconds + 1;
                    else
                ...
                {
                    TimeSpan span = config.LasetServerRestartDate.AddHours(config.HowLongTimeOfRestartServer) - DateTime.Now;
                    double time_temp = span.TotalSeconds % 1;
                    int time;
                    if (time_temp >= 0.5)
                        time = (int)span.TotalSeconds + 1;
        ...
        */
        ...
        private void OnReload(ReloadEventArgs e)
        {
            ...
            string world_name_temp = config.ExpectedUsageWorldFileNameForAotuReset.Count > 0 ? config.ExpectedUsageWorldFileNameForAotuReset.First() : "";
            ...
            {
                for (int i = 1; i < npc.Value.Count; i++)
                {
                    string t = CorrectCommand(npc.Value[i].ToString());

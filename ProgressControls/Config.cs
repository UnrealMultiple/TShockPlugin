...
namespace ProgressControl
{
    public class Config
    {
        ...
        {   //尝试给重复地图编号，其实原版有自动编号的，但是自动编号的地图名称和地图数据的内部名称不一致，你自己手动开服就分不清了
            ...
            while (true)
            {
                if (File.Exists(path() + "/" + name + (count == 1 ? "" : count) + ".wld") && (name + (count == 1 ? "" : count)) != willDelete)
                    count++;
                else if ((name + (count == 1 ? "" : count)) != willDelete)
